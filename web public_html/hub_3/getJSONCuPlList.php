...
class Pdfs
{
    ...
    */
       public function select()
       {
        ...
        if($con != null)
        {
            $result=$con->query("SELECT * FROM currentPlayingList WHERE owner='".$owner."' ORDER BY playlistOrder ASC");
                    ...
                    if($resu2->num_rows>0)
                    {
                        ...
                        while($row2=$resu2->fetch_array())
                        {
                            ...
                            if($spealb->num_rows>0)
                            {
                                ...
                                {
                                    $albn = $roalb['title'];
                                    $albart = $roalb['artworkPath'];
                                }
                            }else
                            {
                                print(json_encode(array("PHP ERROR : for albn")));
                            }
                            array_push($pdfs, array("id"=>$row2['id'],"title"=>$row2['title'],"artist"=>$artn,"album"=>$albn,"genre"=>$row2['genre'],"duration"=>$row2['duration'],"path"=>$row2['path'],"albumOrder"=>$row2['albumOrder'],"plays"=>$row2['plays'],"albart"=>$albart));
                        }
                    }
                }
