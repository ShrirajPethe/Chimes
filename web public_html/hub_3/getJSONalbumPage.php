...
class Pdfs
{
    ...
    */
    public function select()
    {
        ...
        if($con != null)
        {
            $result=$con->query("SELECT * FROM songs WHERE album='".$albumId."' ORDER BY albumOrder ASC");
            if($result->num_rows>0)
            {
                ...
                while($row=$result->fetch_array())
                {
                    ...
                    array_push($pdfs, array("id"=>$row['id'],"title"=>$row['title'],"artist"=>$artn,"album"=>$row['album'],"genre"=>$row['genre'],"duration"=>$row['duration'],"path"=>$row['path'],"albumOrder"=>$row['albumOrder'],"plays"=>$row['plays']));
                }
